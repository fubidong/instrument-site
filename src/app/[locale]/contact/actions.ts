...
/**
 * 提交询价/申请样机线索（前台，统一入口）
 * type: inquiry(询价) | sample(申请样机)
 */
export async function submitLeadAction(
  _prev: InquiryState,
  formData: FormData
): Promise<InquiryState> {
  const type = (formData.get("type") as string) === "sample" ? "sample" : "inquiry";
  ...
  if (!name) return { error: type === "sample" ? "请填写您的姓名" : "请填写您的姓名/称呼" };
  ...
  try {
    await db.inquiry.create({
      data: {
        ...
        message: message || (type === "sample" ? "申请样机" : "（未填写留言）"),
        type,
        status: "pending",
        notifyType: "in_site",
        sourceIp: ip || null,
  ...
  return {
    success: type === "sample" ? "样机申请已提交，我们会尽快与您联系！" : "询价已提交，我们会尽快与您联系！",
