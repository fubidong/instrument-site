...
**已推进（鼎阳产品采集录入，2026-08-31）**：
- [x] 采集鼎阳官网全量产品（scripts/scrape-siglent.mjs 抓 window 数据，880 产品，filter 页 SSR 内嵌）
- [x] 建立 17 个类别（9 大产品线 + 8 子类，含中英翻译 + 品牌分类关联）
- [x] 建立 9 套参数模板（每产品线核心参数，可筛选可对比）
- [x] 导入 75 个鼎阳主仪器系列（排除附件/软件/停产/探头）+ 参数值 337 条
- [x] 下载 74 张产品图到素材库（public/uploads/product/），补全 5 个特殊字符型号图
- [x] 详情页自动抓取官网 og:description 补充 59 个产品摘要
- [x] 参数动态选型：可筛参数改 enum + 自动生成 options（按 | 切分），enum 筛选改 JS token 精确匹配
- [x] 子类继承主类参数模板（filterDefs 取顶层父类）
- [x] 通道数格式归一化（N+EXT → N + EXT）
- [x] 修复 5 个含 / 的型号 model（URL 安全化，显示名保留）
- [x] 前台实测：示波器 23 款列表 + 参数筛选（带宽 8GHz → 3 款正确）+ 子类筛选 + 详情页参数表/询价

**遗留**：15 个产品无官网摘要（英文描述待补）；PDF 资料采集未做（阶段5后续）；泰克/Chroma/普源等品牌待录入（脚本可复用）。
