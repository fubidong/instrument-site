# -*- coding: utf-8 -*-
"""重新批量采集福禄克产品概述（正确HTML，不含技术参数重复）"""
import json, re, os, subprocess, psycopg2
...
def extract_overview_html(html):
    """只提取产品概述部分，到产品规格之前"""
    # 找 overview_content
    m = re.search(r'<div id="overview_content"[^>]*>(.*?)</div>', html, re.DOTALL)
    if m:
        content = m.group(1)
        if len(content) > 50:
            return clean_html(content)
    # 找产品概述到产品规格之间的内容
    start = html.find("产品概述:")
    if start < 0: start = html.find("产品概述")
    if start > 0:
        end = html.find("产品规格", start)
        if end < 0: end = start + 5000
        section = html[start:end]
        # 提取段落、表格、列表
        content = ""
        for pm in re.finditer(r'<(p|table|ul|ol|h[1-6]|img)[^>]*>.*?</\1>|<img[^>]*/?>', section, re.DOTALL):
            content += pm.group(0)
...
cur = conn.cursor()
cur.execute("""SELECT id, model FROM "Product" WHERE "brandId"=%s AND "isActive"=true ORDER BY model""", (BRAND,))
...
with ThreadPoolExecutor(max_workers=5) as executor:
    ...
    for i, future in enumerate(as_completed(futures)):
        ...
        if (i+1) % 40 == 0: print(f"  已处理 {i+1}/{len(db_products)}")

ok = [r for r in results if r[2]]
print(f"\n完成: 成功={len(ok)}")
with open("E:/cxy/_flk_overview_v4.json", "w", encoding="utf-8") as f:
    json.dump([{"pid": r[0], "model": r[1], "overview": r[2]} for r in ok], f, ensure_ascii=False)
print("已保存 _flk_overview_v4.json")
